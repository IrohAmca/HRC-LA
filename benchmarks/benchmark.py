...
def run_ultimate_benchmark():
    wandb.init(project="hrc-la", name=f"Benchmark_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}")
    ...
    std_model_raw = nn.MultiheadAttention(D_MODEL, NUM_HEADS, batch_first=True).to(
        device
    )
    hrc_model = HRCMultiheadAttention(
        D_MODEL, NUM_HEADS, M_FEATURES, batch_first=True
    ).to(device)
    ...
    wandb.log({"benchmark_charts": wandb.Image(fig)})

    print("\nBenchmark charts sent to WandB! 🚀")
    wandb.finish()
